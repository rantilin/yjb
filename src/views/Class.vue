...
<template>
  <div @touchstart="scrollcheck">
    ...
    <div class="gzwechat" v-show="wechatdata">
    ...
    <div :class="[xxflag?'classpanelview':'classpanelviews']">
      <cube-scroll class="classpanels" ref="scroll" :options="options" @pulling-up="onPullingUp">
        <div class="videoview">
          ...
          <div class="content">
            <div class="titleview">
              ...
              <div :class="[collectstate==1?'pic-active':'pic']" @click="colltctionbtn"></div>
            </div>
            <div class="zjbt">第{{playindex+1}}章：{{zjtext}}</div>
            <div class="num">
              <span class="pirce" v-if="goods_price>0">￥{{goods_price}}</span>
              <span class="pricemianfei" v-else>限时免费</span>
              <span class="watch">{{goods_click}}人在看</span>
            </div>
            <div class="geryline"></div>

            <div class="tuijianvideo" v-if="video_active==1&&listdata!=''">
          ...
          <!-- </cube-scroll> -->
            ...
            <!--去找医生浮窗-->
               <div class="tuijiandoctor" v-show="doctoractive" @click="godoctor()">
                  <div class="img-box" > 
            ...
            <div class="geryline" v-if="video_active==1&&listdata!=''"></div>
            <div class="optionbar">
              <div class="vanttab">
                <van-tabs
                  :color="vantab.color"
            ...
            <div class="contentdetail">
              ...
              <div class="slide slide2" v-show="1==currentTab">
                ...
                <!-- audiolist -->
                <cube-button class="moreclass" v-if="videolist.length>morenum" @click="morebtn">
                  ...
                  <li v-for="(item,index) in msglists" :key="index">
                    <div class="cometntdiv" v-if="index<morenumpl">
                      ...
                      <div v-for="(item1,index2) in item.huifu" :key="index2">
                          <div class="user" style="margin-left:35px;margin-top:20px;">
                            <van-image class="pices" fit="cover" :src="item1.jk_image" radius="50%" />
                            <div class="nameview">
                              <div class="name" style="display:flex"><div style="font-weight:bold">{{item1.comment_memberid}}</div><div v-if="item1.comment_memberid=='医教宝'" class="biaoqian">官方</div></div>
                              <div class="time">{{item1.comment_addtime}}</div>
                            </div>
                          </div>
                          <div class="text"  style="margin-left:35px;padding-bottom:10px">{{item1.comment_content}}</div>
                      </div>
      ...
      <div class="mengcengview" @click.self="mengcengview()">
          ...
          <div class="selections">
            <div class="btsses">
              <div class="btss" v-for="(item,index) in videolist" :key="index">
                <cube-button class="ctxtli" :class="{ctxtlis:arr.includes(index)}">
                  <div class="btnviews">
                    <div class="btn1">{{index+1}}</div>
                    <div class="btn2" v-if="item.freession==1">免费</div>
                    <div class="btn2" v-if="item.freession!=1">￥{{item.video_price}}</div>
                  </div>
                  <input
                    class="cks"
                    type="checkbox"
                  ...
                  <div :class="item.freession==1||item.gm==1?'jinyong':'offjinyong'">
                    <div class="btnviewse">
                      ...
                      <div class="btn2e" v-if="item.freession==1">免费</div>
                      <div class="btn2e" v-if="item.freession!=1">已购买</div>
                    </div>
                  </div>
                </cube-button>
              </div>
        ...
        <div class="clear"></div>
          ...
          <div class="codeInput" v-show="DiscountsCode">
                   <div class="code">
                     <input type="text" placeholder="输入您的专属邀请码" v-model="yqmCode" v-on:input="yzcode" :disabled="DiscountsVerify" UIReturnKeyDone @blur="blur">
          ...
          <cube-button class="gobuy" @click="buyclass(classid)" :disabled="disabled">去支付</cube-button>
        </div>
      </div>
    </div>
