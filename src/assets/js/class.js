    ...
    methods: {
        ...
        list1() {
            if (this.key) {
                ...
                classapi.loginvideodetail(this.key, this.classid, this.classtype).then(res => {
                    ...
                    this.isshare = res.data.datas.brief.state_share; //开启自定义分享
                    if (this.isshare == 1) {
                        this.sharedata = res.data.datas.brief.goods_state_share_info
                    }
                    if (this.consult) {
        ...
        bofang() {
            if (!this.key) {
                ...
                if (this.isscreen == 1) {
                    ...
                    element.style.cssText = "height:70vh;"
                }
                if (this.classtype == 2) {
                    this.audioclassimg()
                }
        ...
        list6() {
            if (window.navigator.userAgent.toLowerCase().match(/MicroMessenger/i) == 'micromessenger') {
                if (this.isshare == 1) {
                    share_yp(this.keyval, this.classid, this.sharedata.share_title, this.sharedata.share_image, this.sharedata.share_describes, this.url, this.present);
                } else {
